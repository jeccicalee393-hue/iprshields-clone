...
// Không dùng require() vì subpath '@vercel/blob/client' có thể chỉ export ESM
// trên một số phiên bản/thiết lập -> require() sẽ ném lỗi resolve module,
// khiến function trả về 500 và phía trình duyệt chỉ thấy "Failed to retrieve
// the client token" (không rõ nguyên nhân thật). Dùng import() động để an toàn.
async function getHandleUpload() {
  const mod = await import('@vercel/blob/client');
  return mod.handleUpload;
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    return res.status(400).json({ error: (err && err.message) || 'Lỗi tạo token upload.' });
  }
};
